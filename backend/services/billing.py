...
async def get_stripe_customer_id(client: SupabaseClient, user_id: str) -> Optional[str]:
    ...
    result = await client.table('billing_customers') \
        .select('id') \
    ...
    if customer_result.data and len(customer_result.data) > 0:
        ...
        await client.table('billing_customers').upsert({
            'id': customer['id'],
            'account_id': user_id,
...
async def create_stripe_customer(client, user_id: str, email: str) -> str:
    ...
    await client.table('billing_customers').insert({
        'id': customer.id,
        'account_id': user_id,
        'email': email,
        'provider': 'stripe'
            ...
            try:
                ...
                if commitment_info.get('has_commitment'):
                    if is_upgrade:
                        ...
                        await client.table('billing_customers').update(
                            {'active': True}
